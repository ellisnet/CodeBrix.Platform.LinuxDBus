...
public readonly struct VariantValue : IEquatable<VariantValue>
{
    ...
    /// <summary>Get Object Path.</summary>
    ...
    /// <summary>Get Object Path As String.</summary>
    ...
    /// <summary>Get Dictionary Entry.</summary>
    ...
    public static VariantValue Array(byte[] items)
    {
        ...
        return new VariantValue((IList<byte>)items, nesting: 0);
    }
    /// <summary>Array.</summary>
    ...
    /// <summary>Array Of Variant.</summary>
    ...
    readonly ref struct SignatureCheck
    {
        ...
        private static long GetStructVariantMask(ReadOnlySpan<byte> signature)
        {
            ...
            while (TryReadSingleCompleteType(ref signature, out ReadOnlySpan<byte> itemSignature))
            {
                if (itemSignature.SequenceEqual(CodeBrix.Platform.LinuxDBus.Signature.Variant))
                {
                    mask |= 1L << i;
                }
                i++;
    ...
    private void WriteValueTo(ref MessageWriter writer, int nestingOffset)
    {
        ...
        while (nesting > 1)
        {
            writer.WriteSignature(global::CodeBrix.Platform.LinuxDBus.Signature.Variant);
            nesting--;
        }
        if (nesting == 1)
        {
