...
{
    [Fact(Skip = "Requires Linux (dbus-daemon executable).", SkipUnless = nameof(PlatformGate.IsLinux), SkipType = typeof(PlatformGate))]
    public async Task AddMatchFollowsNameOwner()
    {
        ...
        await signalSemaphore.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        ...
        await nameOwnerChangedSemaphore.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
    ...
    public async Task AddMatchWithSenderUniqueName()
    {
        ...
        await semaphore.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
    ...
    public async Task AddMatchForDBusDoesNotWatchOwner()
    {
        ...
        await nameChangeSemaphore.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
    ...
    public async Task MultipleObserversForSameSender()
    {
        ...
        await semaphore1.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        await semaphore2.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
    ...
    public async Task AddMatchWithoutBusIgnoresSender(bool useUniqueId)
    {
        ...
        var received = await signalReceived.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
