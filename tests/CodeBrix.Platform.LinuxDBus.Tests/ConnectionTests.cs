...
namespace CodeBrix.Platform.LinuxDBus.Tests //was previously: CodeBrix.Platform.LinuxDBus.Tests
{
    public class ConnectionTests
    {
        ...
        public async Task DisposeTriggersRequestAborted()
        {
            ...
            await handler.WaitForRequestReceivedAsync().WaitAsync(timeout, TestContext.Current.CancellationToken);

            conn2.Dispose();

            await Assert.ThrowsAsync<DisconnectedException>(() => pendingCall);

            await handler.WaitForCancelledAsync().WaitAsync(timeout, TestContext.Current.CancellationToken);
        ...
        public async Task ObserverExceptionInValueReaderDisposesObserver()
        {
            ...
            var exception = await exceptionTcs.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        ...
        public async Task HandlerExceptionDisconnectsConnection(bool setSynchronizationContext)
        {
            ...
            var disconnectException = await conn1.DisconnectedAsync().WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
